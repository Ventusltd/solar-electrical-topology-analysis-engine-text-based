"""Authoritative deterministic DuckDB and Parquet topology fleet store."""
...
NULL_TOKEN = "__NULL__"
PAIR_FORMATIONS = "'touching_pair', 'spaced_pair', 'bundled'"
EXTERNAL_TYPES = (
...
SELECT
    ...
    sequential.external_cable_m AS sequential_external_cable_m,
    leapfrog.external_cable_m
        AS leapfrog_external_cable_m_theoretical,
    sequential.external_cable_m
        - leapfrog.external_cable_m AS theoretical_saving_m,
    CASE
...
def _csv_record(segment: SegmentRow) -> dict[str, Any]:
    return {
...
def _write_segment_csv(inputs: TopologyInputs, path: Path) -> int:
...
def _scalar(connection, query: str, parameters=None):
...
def _records(connection, query: str, parameters=None) -> list[dict]:
    cursor = connection.execute(query, parameters or [])
...
def _run_data_law(connection) -> dict[str, int]:
    checks = {
        name: int(_scalar(connection, query))
        for name, query in DATA_LAW_QUERIES.items()
    }
    failures = {
        name: value
    ...
    if failures:
        raise ValueError(f"Topology segment data law failed: {failures}")
    return checks
...
def _copy_query(
    ...
    *,
    ...
    sql_target = _sql_path(target)
    if partitioned:
        connection.execute(
            f"""
            COPY ({query}) TO '{sql_target}' (
                FORMAT parquet,
                COMPRESSION zstd,
...
def _write_aggregates(connection, root: Path) -> None:
    ...
    connection.execute(
        f"CREATE TABLE string_aggregates AS {STRING_AGGREGATE_SQL}"
    )
    connection.execute(
        f"CREATE TABLE mppt_aggregates AS {MPPT_AGGREGATE_SQL}"
    )
    connection.execute(
        f"CREATE TABLE inverter_aggregates AS {INVERTER_AGGREGATE_SQL}"
    )
    connection.execute(
        f"CREATE TABLE site_aggregates AS {SITE_AGGREGATE_SQL}"
    )
    connection.execute(
        f"CREATE TABLE comparison_aggregate AS {COMPARISON_SQL}"
    )

    exports = (
        (
            "SELECT * FROM string_aggregates "
            "ORDER BY topology, string_id",
            "strings.parquet",
        ),
        (
            "SELECT * FROM mppt_aggregates "
            "ORDER BY topology, inverter_id, mppt_id",
            "mppts.parquet",
        ),
        (
            "SELECT * FROM inverter_aggregates "
    ...
    for query, filename in exports:
        _copy_query(connection, query, aggregate_root / filename)
...
def _topology_file_hashes(root: Path, topology: str) -> dict[str, str]:
    marker = f"topology={topology}"
    return {
        path.relative_to(root).as_posix(): _sha256(path)
...
def _write_manifests(
    ...
    for cartridge in INITIAL_CARTRIDGES:
        ...
        if statistics is None:
            raise ValueError(
                f"No segment statistics for {cartridge.name}"
            )
        topology_files = _topology_file_hashes(root, cartridge.name)
        ...
        manifest = {
            ...
            "method_version": "duckdb_parquet_store_v2",
...
def _load_segments(connection, csv_path: Path) -> None:
...
def build_store(
    ...
    """Build one candidate segment, result, aggregate and browser store."""
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            _load_segments(connection, csv_path)
            loaded_rows = int(
                _scalar(connection, "SELECT count(*) FROM segments")
            )
            if generated_rows != loaded_rows:
                raise ValueError(
                    "Generated and loaded segment row counts differ"
                )
...
def _determinism_error(
    ...
    missing_first = sorted(set(second_hashes) - set(first_hashes))
    missing_second = sorted(set(first_hashes) - set(second_hashes))
    changed = sorted(
        path
...
def build_deterministic_store(
    ...
    """Build twice and publish only byte-identical relative outputs."""
    ...
    with tempfile.TemporaryDirectory(
        ...
        if first_hashes != second_hashes:
            raise ValueError(
                _determinism_error(first_hashes, second_hashes)
            )
        if output_root.exists():
            shutil.rmtree(output_root)
